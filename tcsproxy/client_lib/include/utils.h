 ...
 * $Id: utils.h,v 1.14 1997/09/06 02:29:40 gribble Exp $
 * 
 */
...
const char *strncasestr(const char *string, const char *substr, size_t strlen);

/*
 * A version of strncpy which automatically appends a '\0' at the end, 
 * if required
 */
 ...
 * until all len bytes have been written.  correct_write_to will
 * time out if no progress is made for more than timeout seconds.
 * If a timeout occurs, correct_write returs the number of bytes written,
 * and sets the timed_out flag to 1.  Otherwise, timed_out if set to 0.
 */
...
/*
 * make_nonblocking will turn a socket into a non-blocking socket.  1
...
/*
 * make_blocking will turn a socket into a blocking socket.  1
 * for success, 0 for failure.
 */
