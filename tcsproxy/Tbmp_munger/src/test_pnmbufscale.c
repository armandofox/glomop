...
/*  SET_ARG_ID(args[nargs], PNMSCALE_XSCALE);
  SET_ARG_DOUBLE(args[nargs], 0.5);
  nargs++;
  SET_ARG_ID(args[nargs], PNMSCALE_YSCALE);
  SET_ARG_DOUBLE(args[nargs], 0.75);
  nargs++;*/
  SET_ARG_ID(args[nargs], PNMSCALE_XSIZE);
  SET_ARG_INT(args[nargs], 49);
  nargs++;
  SET_ARG_ID(args[nargs], PNMSCALE_YSIZE);
  SET_ARG_INT(args[nargs], 189);
  nargs++;
  ...
  while (fgets(nextfile, 254, f) != NULL) {
    ...
    st = pnmscale_DistillerMain(args,nargs,&in,&out);
    if (st != distOk) {
      fprintf(stderr, "pnmscale_DistillerMain failed: error %d\n", (int)st);
    }
    close(fd);
