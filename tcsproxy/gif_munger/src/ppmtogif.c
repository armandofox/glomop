...
static void GIFEncode ARGS(( const void *outbuf, unsigned long *buflen, int GWidth, int GHeight, int GInterlace, int Background, int Transparent, int BitsPerPixel, int* Red, int* Green, int* Blue, ifunptr GetPixel ));
int Red[MAXCOLORS],Green[MAXCOLORS],Blue[MAXCOLORS],perm[MAXCOLORS],permi[MAXCOLORS];
int colors;
pixval maxtmp;
static void compress ARGS(( int init_bits, ifunptr ReadValue ));
...
unsigned long
ppmtogif_main(int interlace, int sort, int transparent,
              pixel *transcolorpixel,
              ppmbuf *pbuf, void *outbuf)
{
    ...
    if (transparent>0)
      transcolor = *transcolorpixel;

    pixels = pbuf->rowarray;
    rows= pbuf->rows;
    cols= pbuf->cols;
    maxval = maxtmp = (pixval)255;
    ...
    for (i=0;i<colors;i++)
      permi[i]=i;
    if (sort) {
      /*      fprintf(stderr,"sorting colormap"); */
      for (i=0;i<colors;i++)
    ...
    GIFEncode(
              outbuf, &outbuflen,
              cols, rows, interlace, 0, transparent, BitsPerPixel,
 ...
 *
 * GIFEncode( void *outbuf, unsigned long *outbuflen [value-result],
 *              GHeight, GWidth, GInterlace, Background, Transparent,
...
/*
 * Write out a word or byte to the GIF file
 */

#define Putword(w) *(Fp++)=(w)&0xff; *(Fp++)=((w)>>8)
#define PutByte(w) *(Fp++)=(w)&0xff

static void
GIFEncode( outbuf, outbuflen,
           GWidth, GHeight, GInterlace, Background, Transparent,
           BitsPerPixel, Red, Green, Blue, GetPixel )
...
ifunptr GetPixel;
{
        ...
        int InitCodeSize;
        int i;
        extern void gifencode_init(void);
        
        gifencode_init();
        
        Fp = (unsigned char *)outbuf;
         ...
         */
        memcpy((void*)Fp, (Transparent < 0 ? "GIF87a" : "GIF89a"), (size_t)6);
        Fp += 6;
	...
	if ( Transparent >= 0 ) {
         ...
         */
        compress( InitCodeSize+1, GetPixel );
         ...
         */
        PutByte( ';' );
        *outbuflen = (unsigned long)(Fp - (unsigned char *)outbuf);
}
...
static const int maxbits = BITS; /* user settable max # bits/code */
static code_int maxcode;                  /* maximum code, given n_bits */
static const code_int maxmaxcode = (code_int)1 << BITS; /* should NEVER generate this code */
...
{
        if( a_count > 0 ) {
                *(Fp++) = (unsigned char)a_count;
                memcpy((void *)Fp, (const void *)accum, (size_t)a_count);
                Fp += a_count;
                a_count = 0;
        }
}
