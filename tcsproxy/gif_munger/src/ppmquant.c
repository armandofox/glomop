...
** $Id: ppmquant.c,v 1.3 1997/04/26 01:31:56 fox Exp $
...
*/
static int redcompare ARGS(( const void * ch1, const void * ch2 ));
static int greencompare ARGS(( const void * ch1, const void * ch2 ));
static int bluecompare ARGS(( const void * ch1, const void * ch2 ));
static int sumcompare ARGS(( const void * b1, const void * b2 ));
    ...
    for ( row = 0; row < rows; ++row )
      {
          ...
          do
	    {
                ...
                /* do not modify pixel if it was the "transparent" color */

                if (! PPM_EQUAL(*pP, *transcolorpixel))
...
static int
redcompare( const void *c1, const void *c2 )
{
     colorhist_vector ch1, ch2;
     ch1 = (colorhist_vector)c1;
     ch2 = (colorhist_vector)c2;
    return (int) PPM_GETR( ch1->color ) - (int) PPM_GETR( ch2->color );
}

static int
greencompare( const void *c1, const void *c2 )
{
     colorhist_vector ch1, ch2;
     ch1 = (colorhist_vector)c1;
     ch2 = (colorhist_vector)c2;
...
static int
bluecompare( const void *c1, const void *c2 )
{
     colorhist_vector ch1, ch2;
     ch1 = (colorhist_vector)c1;
     ch2 = (colorhist_vector)c2;
...
static int
sumcompare( const void *vb1, const void *vb2 )
{
     box_vector b1, b2;
     b1 = (box_vector)vb1;
     b2 = (box_vector)vb2;
