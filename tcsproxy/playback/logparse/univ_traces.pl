...
#    &do_convert($nextfile, "/disks/barad-dur/sandbox1/gribble/clienttraces/dcns", "1");
#}

#foreach $nextfile (@dcns2) {
#    &do_convert($nextfile, "/disks/barad-dur/sandbox1/gribble/clienttraces/dcns2", "2");
#}

foreach $nextfile (@dcns3) {
    &do_convert($nextfile, "/disks/barad-dur/sandbox1/gribble/clienttraces/dcns3", "3");
}
...
sub do_convert {
    ...
    my $vers = shift(@_);
    my ($copycmd, $convertcmd, $copybackcmd, $rmcmd, $file2);

    $file2 = $file;  chop($file2);  chop($file2); chop($file2);
    $copycmd = "rcp latte.cs.berkeley.edu:${remotepath}/${file} .";
    system($copycmd);
    $convertcmd = "gunzip $file; ./httpde_univ $vers < $file2 | gzip > ${file2}.univ.gz";
    system($convertcmd);
    $copybackcmd = "rcp ${file2}.univ.gz latte.cs.berkeley.edu:${remotepath}/${file2}.univ.gz";
    system($copybackcmd);
    $rmcmd = "/bin/rm -f $file2 ${file2}.univ.gz";
    system($rmcmd);
}
